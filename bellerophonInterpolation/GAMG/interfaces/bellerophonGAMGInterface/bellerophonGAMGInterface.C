...
#include "bellerophonOldInterpolation.H"
...
namespace Foam
{
    defineTypeNameAndDebug(bellerophonGAMGInterface, 0);
...
Foam::bellerophonGAMGInterface::bellerophonGAMGInterface
(
    const label index,
...
:
    ...
    fineBellerophonInterface_
    (
        refCast<const bellerophonInterface>(fineInterface)
    )
{
    Info<< "bellerophonGAMGInterface::bellerophonGAMGInterface(...)"
        << " at bellerophonGAMGInterface.C:47" << nl << endl;
    ...
    // On the owner side construct the bellerophon

    if (fineBellerophonInterface_.owner())
    {
        ...
        amiPtr_.reset
        (
            new bellerophonPatchToPatchInterpolation
            (
                fineCyclicbellerophonInterface_.bellerophon(),
...
Foam::tmp<Foam::labelField> Foam::bellerophonGAMGInterface::internalFieldTransfer
(
    const Pstream::commsTypes,
    const labelUList& iF
) const
{
    Info<< "bellerophonGAMGInterface::internalFieldTransfer(...)"
        << " at bellerophonGAMGInterface.C:179" << nl << endl;

    const bellerophonGAMGInterface& nbr =
        dynamic_cast<const bellerophonGAMGInterface&>(neighbPatch());
